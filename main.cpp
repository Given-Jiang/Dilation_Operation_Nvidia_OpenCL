...
//	img_buf.create(576, 720, CV_8UC1);
	img_buf = imread(str);
	GaussianBlur(img_buf, img_buf, Size(5, 5), 0, 0, BORDER_DEFAULT);
	cvtColor(img_buf, img_buf, CV_BGR2GRAY);
	threshold(img_buf, img_buf, 0, 255, CV_THRESH_OTSU + CV_THRESH_BINARY);
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	//make prograam 
	cl_program program = load_program(context, "..\\Dilation.cl");
	if(program == 0) 
	{
	...
	//create kernel
	cl_kernel dilation = clCreateKernel(program, "dilation", 0);
	if(dilation == 0)
	{
	...
	clSetKernelArg(dilation, 0, sizeof(cl_mem), &cl_img_in);
	clSetKernelArg(dilation, 1, sizeof(cl_mem), &cl_img_out);
	clSetKernelArg(dilation, 2, sizeof(unsigned int), &DATA_SIZE);
...
/*	for ( int xx = 0; xx < 720 * 576 ; xx++)
	...
	size_t work_size = 1;
	cl_event event;
	err = clEnqueueNDRangeKernel(queue, dilation, 1, 0, &work_size, 0, 0, 0, &event);
	clFinish(queue);
	...
	morphologyEx(in_img, dst, MORPH_DILATE, element);
	//time count over
